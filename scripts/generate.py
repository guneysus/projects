...
def build_project_page(project, duplicates, source_info=None, prev=None, next_=None):
    """Render a single project markdown page.

    source_info : dict with core_idea / snippet / snippet_lang (from source_reader)
    prev / next_ : dicts with 'name' and 'path' for navigation links
    """
    name = project["name"]
    ...
    # Core Idea section (from source_reader)
    if source_info and source_info.get("core_idea"):
        lines += [
            "",
            "## 💡 Core Idea",
            "",
            source_info["core_idea"],
            "",
        ]

    # Code Snippet section (for small projects)
    if source_info and source_info.get("snippet"):
        lang = source_info.get("snippet_lang") or ""
        lines += [
            "## 🔍 Code Snippet",
            "",
            f"```{lang}",
    ...
    nav = []
    if prev:
        nav.append(f"[← {prev['name']}]({prev['path']})")
    if next_:
        nav.append(f"[{next_['name']} →]({next_['path']})")
    if nav:
...
def main():
    ...
    from source_reader import read_project_source

    # Build the ordered list of canonical pages (tier -> category -> name) for
    # next/prev navigation. Forks are ordered separately.
    def page_path(p):
        slug = slugify(p["name"])
        if p.get("fork"):
    ...
    def nav_key(p):
        ...
        return (0, TIER_ORDER.index(tier_of(p)), CATEGORIES[category_of(p)]["label"].lower(), p["name"].lower())

    ordered = sorted(originals, key=nav_key) + sorted(forks, key=nav_key)
    nav_index = {id(p): i for i, p in enumerate(ordered)}
    ...
    for p in sorted(projects, key=lambda x: (not x.get("canonical", True), x["name"].lower())):
        ...
        # Source info (core idea + snippet) from the gitlab-archive
        source_info = read_project_source(p) if not p.get("duplicate_of") else {}

        # Next/prev navigation
        prev = next_ = None
        if id(p) in nav_index:
            i = nav_index[id(p)]
            if i > 0:
                prev_p = ordered[i - 1]
